  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (isEdit) {
        ...
        await api.post(`/units/${targetUnit}/contracts`, data);
      }
      onSuccess();
      onClose();
    } catch (err: any) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            {errors.rent_amount && <p className="mt-1 text-xs text-red-500">{errors.rent_amount.message}</p>}
          </div>
          <div>
        ...
        <div>
          ...
          <textarea {...register('terms')} rows={3} className="input" placeholder="Contract terms and conditions…" />
        </div>
